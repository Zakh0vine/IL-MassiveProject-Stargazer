        ...
        <div className={`${toggle ? "flex" : "hidden"} p-[25px] bg-[#d3e3ff] absolute top-24 right-0 mx-4 my-1 min-w-[140px] rounded-xl sidebar`}>
          <ul className="list-none ">
            {sideNav.slice(0, 5).map((nav, index) => (
              <li key={nav.name} className={`font-poppins font-semibold cursor-pointer text-black hover:text-[#5d6af8] text-[20px]  ${index === sideNav.length - 1 ? "mb-0" : "mb-6"} flex gap-4`}>
